    ...
    while True:
        try:
            ...
            with torch.no_grad():
                ...
                lyap_exp_np = None
                d_end = None
                # --- 1. DIVERGENCE METRIC (MAX PATCH) ---
                if b_size > 1:
                    z_visual = z_obses['visual'] # Shape: (B, T, 392, 384)
                    n_hist = visual_t.shape[1]
                    
                    # Extract ONLY the fixed front camera (Assuming it's view index 1, patches 196:392)
                    ...
                    # Find the MAXIMUM patch distance (Most Divergent Patch)
                ...
                if has_decoder:
                    # decode_obs returns a tuple: (obs_dict, diff)
                    decoded_obs, _ = model.decode_obs(z_obses)
                    pred_visual = decoded_obs['visual'] # Shape: (B, T, V, C, H, W)
            ...
            socket.send_pyobj({
                'states': result_images,      # Send the decoded images back!
                'lyapunov': d_end.cpu().numpy(),
