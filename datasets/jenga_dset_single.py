...
class LazyVideo:
    ...
    def __init__(self, lmdb_env, episode_keys, cam_prefixes, num_frames, transform=None):
        ...
        # Set dynamic shape based on camera count
        if len(self.cam_prefixes) == 1:
            self.shape = (num_frames, 3, 224, 224)
        else:
    ...
    def __getitem__(self, item):
        ...
        # Squeeze the view dimension if there is only 1 camera
        if len(self.cam_prefixes) == 1:
            images = images.squeeze(1)

        if self.transform:
            if images.ndim == 4: # Single View: (t, c, h, w)
                images = self.transform(images)
            else: # Multi-View: (t, v, c, h, w)
...
class JengaDataset(TrajDataset):
    def __init__(self, data_path, n_rollout=None, transform=None, normalize_action=True, cam_prefixes=None):
        ...
        self.lmdb_path = self.data_path / "jenga_single.lmdb"
        self.transform = transform
        self.normalize_action = normalize_action
        self.cam_prefixes = cam_prefixes or ["cam2"] # Default to just cam2
...
def load_jenga_slice_train_val(
    ...
    split_ratio=0.9, num_hist=0, num_pred=0, frameskip=1, action_freq=10, img_freq=10, cam_serials=None, cam_prefixes=["cam2"], **kwargs
):
    dset = JengaDataset(data_path, n_rollout, transform, normalize_action, cam_prefixes=cam_prefixes)
